  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                ...
                $name = "";
                if(isset($Row[1])) {
                    $fristname = mysqli_real_escape_string($conn,$Row[1]);
                }
                ...
                if (!empty($fristname) || !empty($lastname)) {
                    $query ="INSERT INTO `student`(`full_name`, `total`,`id_college`, `id_departement`) 
                     VALUES
                    ('{$fristname}','{$lastname}','{$department}','{$college}')";
                    $result = mysqli_query($conn, $query);
