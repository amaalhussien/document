		...
		<table class="main-table text-center table table-bordered" >
			...
			while($row=mysqli_fetch_assoc($result)){
					...
					echo "<tr>";
						...
						echo "<td>
							<a href='employee.php?do=Edit&id_dep=".$row['id_employee'] . "' class='btn btn-success' ><i class='fa fa-edit'></i> Edit</a>
							<a href='employee.php?do=Delete&id_dep=" . $row['id_employee'] . "' class='btn btn-danger confirm s><i class='fa fa-close'></i> Delete </a>";
					  echo "</td>";
					 echo "</tr>";
				}
			?>
...
<?php
    ...
    if(!empty($errors)){
        ...
        redicrt("?do=Add");
		 ...
		 VALUES ('{$username}','{$email}','{$pass1}','{$status}','{$college}',0)";
					...
					<input type="hidden" name="id_dep" value="<?php echo $id_dep ?>" />
						...
						<?php
                                 ...
                                 foreach ($row1 as $u){
                                    ...
                                    if ($row['college_id_college']== $u['id_College']) 
                                    { 
                                        echo 'selected';
 ...
 <?php
            ...
            if(isset($_POST['submit'])){
                ...
                $sql="UPDATE `department` SET
                            ...
                            `college_id_college`='{$college}' WHERE `id_department`='{$id}'
                            ";
                        if (mysqli_query($conn, $sql) && mysqli_affected_rows($conn)>0) {
                        $_SESSION['msg']=secusse_msg_admin();
                        redicrt("?do=Manage");
